				...
				IShaderShadow* pShaderShadow, PaintBlobVars_t &info, VertexCompressionType_t vertexCompression )
{
	...
	bool bFlattenStaticControlFlow = true;//!g_pHardwareConfig->SupportsStaticControlFlow();
	...
	DYNAMIC_STATE
	{
		...
		if ( lightState.m_nNumLights > 2 )
		{
			AssertMsg( lightState.m_nNumLights < 2, "Find a real solution to this problem instead of forcefully changing numlights to 2" );
			lightState.m_nNumLights = 2; // RETRACT: HACK
		}
